...
async def verify_api_key(credentials: Annotated[HTTPBasicCredentials, Depends(security)], db=Depends(get_db_connection), redis=Depends(get_redis_client)):
    ...
    expected_api_key = redis.get(f"PYTOGO_API_KEY:{credentials.credentials}")
    if not expected_api_key:
        ...
        # Cache for 1 hour
        await redis.set(f"PYTOGO_API_KEY:{credentials.credentials}", dumps(expected_api_key), ex=3600)

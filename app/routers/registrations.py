...
@api_router.post("/student/register/{event_code}", status_code=status.HTTP_201_CREATED)
async def register_student(request: Request, registration: StudentProof, event_code: str, background_tasks: BackgroundTasks, db=Depends(get_db_connection)):
    """
    ...
    try:
        ...
        studen_proof = {
            ...
            "file_url": registration.student_proof_url,
        }
        payment_data = {
            "amount": registration.ticket_price * registration.quantity,
            "callback_url": f"{base_url}/webhooks/paydunya/callback",
        ...
        async with httpx.AsyncClient() as client:
            ...
            background_tasks.add_task(
                create_registration, db, registration_data, True)
